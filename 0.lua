if getgenv().CrazyHubLoaded then
    if getgenv().CrazyHubUI and getgenv().CrazyHubUI.Parent then
        pcall(function()
            getgenv().CrazyHubUI:Destroy()
        end)
    end
    getgenv().CrazyHubRunning = false
    task.wait(0.25)
end

getgenv().CrazyHubLoaded = true
getgenv().CrazyHubRunning = true
...
local Window = Library:CreateWindow({
    Title = "Crazy Hub",
    Footer = "Anime Eternal - by Crazy00z",
    Icon = 95816097006870,
    NotifySide = "Right",
...
getgenv().CrazyHubUI = Library.Gui
...
WatermarkGui.Name = "CrazyWatermark"
...
-- Make watermark draggable and clickable
...
local function onInputBegan(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        ...
        local inputChangedConnection = UserInputService.InputChanged:Connect(function(globalInput)
        ...
        local inputEndedConnection = UserInputService.InputEnded:Connect(function(globalInput)
...
-- Configuration
local configFolder = "CrazyHub"
local configFile = configFolder .. "/crazy_hub_AE.txt"

if not isfolder(configFolder) then
    makefolder(configFolder)
end

local config = getgenv().CrazyHubConfig or {}
...
local autoRedeemCodesEnabled = config.AutoRedeemCodesToggle or false
...
-- Stats options and mappings
local stats = {"Damage", "Energy", "Coins", "Luck"}
local statMap = {
...
local Tabs = {
    ...
    Teleport = Window:AddTab("Teleport & Dungeon", "map"),
    Rolls = Window:AddTab("Rolls", "dice-5"),
    Upgrades = Window:AddTab("Upgrades", "arrow-up"),
    ["UI Settings"] = Window:AddTab("UI Settings", "settings"),
}

-- Main Tab Groups
local LeftGroupbox = Tabs.Main:AddLeftGroupbox("Automation", "zap")
local StatsGroupbox = Tabs.Main:AddLeftGroupbox("Auto Stats", "trending-up")
local RewardsGroupbox = Tabs.Main:AddRightGroupbox("Auto Rewards", "gift")

-- Teleport Tab Groups  
local TPGroupbox = Tabs.Teleport:AddLeftGroupbox("Main Teleport", "map-pin")
local DungeonGroupbox = Tabs.Teleport:AddRightGroupbox("Auto Dungeon", "shield")

-- Rolls Tab Groups
local RollGroupbox = Tabs.Rolls:AddLeftGroupbox("Auto Rolls", "dice-1")
local RollGroupbox2 = Tabs.Rolls:AddLeftGroupbox("Auto Roll Tokens", "dice-2")
local AutoDeleteGroupbox = Tabs.Rolls:AddRightGroupbox("Auto Delete Stars", "trash-2")
local AutoDeleteGroupbox2 = Tabs.Rolls:AddRightGroupbox("Auto Delete Gacha", "x")

-- Upgrades Tab Groups
local UpgradeGroupbox = Tabs.Upgrades:AddLeftGroupbox("Upgrades", "arrow-up")
local Upgrade2 = Tabs.Upgrades:AddRightGroupbox("Upgrades 2", "arrow-up-circle")

-- UI Settings Tab Groups
local UnloadGroupbox = Tabs["UI Settings"]:AddLeftGroupbox("Utilities", "wrench")
local RedeemGroupbox = Tabs["UI Settings"]:AddRightGroupbox("Redeem Codes", "gift")
local UISettingsGroup = Tabs["UI Settings"]:AddLeftGroupbox("UI Customization", "palette")
local InfoGroup = Tabs["UI Settings"]:AddRightGroupbox("Script Information", "info")
...
local function startAutoFarm()
    task.spawn(function()
        while getgenv().CrazyHubRunning and isAuraEnabled do
...
local function startFastKillAura()
    task.spawn(function()
        ...
        while fastKillAuraEnabled and getgenv().CrazyHubRunning do
            local char = localPlayer.Character
            local myHRP = char and char:FindFirstChild("HumanoidRootPart")
...
local function startSlowKillAura()
    task.spawn(function()
        ...
        while slowKillAuraEnabled and getgenv().CrazyHubRunning do
...
local function startAutoRank()
    task.spawn(function()
        while autoRankEnabled and getgenv().CrazyHubRunning do
            local args = {
                [1] = {
                    ["Upgrading_Name"] = "Rank",
                    ["Action"] = "_Upgrades",
...
local function startAutoAvatarLeveling()
    task.spawn(function()
        while getgenv().CrazyHubRunning and autoAvatarLevelingEnabled do
            local args = {
                [1] = {
                    ["Value"] = true,
                    ["Path"] = {
...
local function startAutoQuests()
    task.spawn(function()
        while autoAcceptAllQuestsEnabled and getgenv().CrazyHubRunning do
            for questId = 1, 99 do
                local argsAccept = {
                    [1] = {
                        ["Id"] = tostring(questId),
...
local function startAutoAchievements()
    task.spawn(function()
        ...
        while autoClaimAchievementsEnabled and getgenv().CrazyHubRunning do
...
local function startAutoRollDragonRace()
    task.spawn(function()
        ...
        while getgenv().CrazyHubRunning and autoRollDragonRaceEnabled do
            local args = {
                [1] = {
                    ["Open_Amount"] = 1,
...
local function startAutoRollSaiyanEvolution()
    task.spawn(function()
        ...
        task.wait(2)
        while getgenv().CrazyHubRunning and autoRollSaiyanEvolutionEnabled do
            local args = {
                [1] = {
                    ["Open_Amount"] = 1,
                    ["Action"] = "_Gacha_Activate",
...
local function startAutoRollStars()
    task.spawn(function()
        while getgenv().CrazyHubRunning and autoRollEnabled do
            pcall(function()
                local cacheArgs = {
                    [1] = {
...
local function startAutoDelete()
    task.spawn(function()
        while autoDeleteEnabled and getgenv().CrazyHubRunning do
            pcall(function()
                local starRarityMap = {
                    ["Star_1"] = {
                        ["1"] = {"70001", "70008"},
...
local function startAutoTimeReward()
    task.spawn(function()
        while isAutoTimeRewardEnabled and getgenv().CrazyHubRunning do
            local args = {
                [1] = {
...
local function startAutoDailyChest()
    task.spawn(function()
        while isAutoDailyChestEnabled and getgenv().CrazyHubRunning do
            local args = {
                [1] = {
                    ["Action"] = "_Chest_Claim",
                    ["Name"] = "Daily",
...
local function startAutoVipChest()
    task.spawn(function()
        while isAutoVipChestEnabled and getgenv().CrazyHubRunning do
            local args = {
                [1] = {
                    ["Action"] = "_Chest_Claim",
                    ["Name"] = "Vip",
...
local function startAutoGroupChest()
    task.spawn(function()
        while isAutoGroupChestEnabled and getgenv().CrazyHubRunning do
            local args = {
                [1] = {
                    ["Action"] = "_Chest_Claim",
                    ["Name"] = "Group",
...
local function startAutoPremiumChest()
    task.spawn(function()
        while isAutoPremiumChestEnabled and getgenv().CrazyHubRunning do
            local args = {
                [1] = {
                    ["Action"] = "_Chest_Claim",
                    ["Name"] = "Premium",
...
local function startAutoEnterDungeon()
    task.spawn(function()
        while autoEnterDungeon and getgenv().CrazyHubRunning do
...
local function startAutoUpgrade()
    task.spawn(function()
        while autoUpgradeEnabled and getgenv().CrazyHubRunning do
            pcall(function()
...
local function startAutoRollSwords()
    task.spawn(function()
        ...
        while getgenv().CrazyHubRunning and autoRollSwordsEnabled do
            local rollArgs = {
                [1] = {
                    ["Open_Amount"] = 1,
                    ["Action"] = "_Gacha_Activate",
...
local function startAutoRollPirateCrew()
    task.spawn(function()
        ...
        while getgenv().CrazyHubRunning and autoRollPirateCrewEnabled do
            local rollArgs = {
                [1] = {
                    ["Open_Amount"] = 1,
                    ["Action"] = "_Gacha_Activate",
...
local function startAutoHakiUpgrade()
    task.spawn(function()
        local unlockArgs = {
            [1] = {
                ["Upgrading_Name"] = "Unlock",
        ...
        while autoHakiUpgradeEnabled and getgenv().CrazyHubRunning do
            local upgradeArgs = {
                [1] = {
...
local function startAutoRollDemonFruits()
    task.spawn(function()
        local unlockArgs = {
            [1] = {
                ["Upgrading_Name"] = "Unlock",
        ...
        while autoRollDemonFruitsEnabled and getgenv().CrazyHubRunning do
            local rollArgs = {
                [1] = {
                    ["Open_Amount"] = 1,
                    ["Action"] = "_Gacha_Activate",
...
local function startAutoAttackRangeUpgrade()
    task.spawn(function()
        local unlockArgs = {
            [1] = {
                ["Upgrading_Name"] = "Unlock",
        ...
        while autoAttackRangeUpgradeEnabled and getgenv().CrazyHubRunning do
            local upgradeArgs = {
                [1] = {
...
local function startAutoSpiritualPressureUpgrade()
    task.spawn(function()
        local unlockArgs = {
            [1] = {
                ["Upgrading_Name"] = "Unlock",
        ...
        task.wait(2)
        while autoSpiritualPressureUpgradeEnabled and getgenv().CrazyHubRunning do
            local upgradeArgs = {
                [1] = {
...
local function startAutoRollReiatsuColor()
    task.spawn(function()
        local unlockArgs = {
            [1] = {
                ["Upgrading_Name"] = "Unlock",
        ...
        while autoRollReiatsuColorEnabled and getgenv().CrazyHubRunning do
            local rollArgs = {
                [1] = {
                    ["Open_Amount"] = 1,
                    ["Action"] = "_Gacha_Activate",
...
local function startAutoRollZanpakuto()
    task.spawn(function()
        ...
        while autoRollZanpakutoEnabled and getgenv().CrazyHubRunning do
            local rollArgs = {
                [1] = {
                    ["Open_Amount"] = 1,
                    ["Action"] = "_Gacha_Activate",
...
local function startAutoCursedProgressionUpgrade()
    task.spawn(function()
        local unlockArgs = {
            [1] = {
                ["Upgrading_Name"] = "Unlock",
        ...
        while autoCursedProgressionUpgradeEnabled and getgenv().CrazyHubRunning do
            local upgradeArgs = {
                [1] = {
...
local function startAutoRollCurses()
    task.spawn(function()
        ...
        while autoRollCursesEnabled and getgenv().CrazyHubRunning do
            local rollArgs = {
                [1] = {
                    ["Open_Amount"] = 1,
                    ["Action"] = "_Gacha_Activate",
...
local function startAutoObelisk()
    task.spawn(function()
        local obeliskTypes = {
            "Dragon_Obelisk",
            "Slayer_Obelisk",
        ...
        while autoObeliskEnabled and getgenv().CrazyHubRunning do
...
local function startAutoRollDemonArts()
    task.spawn(function()
        ...
        while autoRollDemonArtsEnabled and getgenv().CrazyHubRunning do
            local rollArgs = {
                [1] = {
                    ["Open_Amount"] = 1,
                    ["Action"] = "_Gacha_Activate",
...
local function startAutoDeleteGacha()
    task.spawn(function()
        while config.AutoDeleteGachaUnitsToggle and getgenv().CrazyHubRunning do
...
-- Script Information
InfoGroup:AddLabel("Script by: Crazy00z")
InfoGroup:AddLabel("Version: 2.0.0")
InfoGroup:AddLabel("Game: Anime Eternal")
InfoGroup:AddLabel("Created: 2025-08-18")
...
-- Unload Button
UnloadGroupbox:AddButton("Unload Crazy Hub", function()
    getgenv().CrazyHubRunning = false
    isAuraEnabled = false
    ...
    elseif getgenv().CrazyHubUI and getgenv().CrazyHubUI.Parent then
        pcall(function()
            getgenv().CrazyHubUI:Destroy()
        end)
    end
    ...
    getgenv().CrazyHubUI = nil
    getgenv().CrazyHubLoaded = nil
    getgenv().CrazyHubRunning = nil
    getgenv().CrazyHubConfig = nil
...
ThemeManager:SetFolder("CrazyHub")
SaveManager:SetFolder("CrazyHub/configs")
...
task.defer(function()
    repeat task.wait() until Library.Options
...
Library:OnUnload(function()
    getgenv().CrazyHubRunning = false
    print("Crazy Hub unloaded!")
end)

Library:Notify({
    Title = "Crazy Hub",
    Description = "Script loaded successfully! All features from Seisen Hub converted to Obsidian UI.",
    Time = 5,
})
end 

print("🔥 Crazy Hub v2.0 loaded successfully by Crazy00z!")
print("📋 All features converted and ready to use!")